				...
				<scroll-view scroll-x class=" nav">
					<view class="flex text-center text-c-msg">
						<view class="cu-item flex-sub"
							:class="index == currentIndex ? 'theme-color font-weight-act' : 'font-weight-rg'"
							v-for="(item, index) in tabList" :key="index" @tap="tabSelect(index, item)" :data-id="index">
							{{ item.name }}
						</view>
					</view>
				</scroll-view>
...
import { getCommendApplyRecord } from '../../api/commend.js'
...
export default defineComponent({
	...
	setup(props) {
		...
		const tabSelect = (index, item) => {
			console.log(index)
			currentIndex.value = index
			paging.value.reload()
			YWJATRACK.uploadTrack('我的申请页面' + item.name, 'key5')
		}
		...
		onLoad(() => {
			YWJATRACK.uploadTrack('我的申请页面', 'key4')
		})
