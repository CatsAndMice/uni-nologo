...
export const submitColleaguesLive = async (params) => {
    const [err, result] = await to(request.post('/commendation/apply/like', params))
    if (isEmpty(result)) return false
    return result.code === 200
}

export const submitDeptLive = async (params) => {
    const [err, result] = await to(request.post('/commendation/apply/dept', params))
    if (isEmpty(result)) return false
    return result.code === 200
}

export const getDept = async () => {
    const [err, result] = await to(request.get('/dept/energy'))
